...
** wrong.
*/
int winRead(OsFile *id, void *pBuf, int amt){
  DWORD got;
  assert( id!=0 );
...
*/
int winWrite(OsFile *id, const void *pBuf, int amt){
  int rc = 0;
  DWORD wrote;
  assert( id!=0 );
  SimulateIOError(SQLITE_IOERR);
...
*/
int winSeek(OsFile *id, i64 offset){
...
*/
int winSync(OsFile *id, int dataOnly){
  assert( id!=0 );
...
*/
int winTruncate(OsFile *id, i64 nByte){
...
*/
int winFileSize(OsFile *id, i64 *pSize){
...
** Undo a readlock
*/
int unlockReadLock(winFile *pFile){
  int res;
  if( isNT() ){
...
*/
int winLock(OsFile *id, int locktype){
...
*/
int winCheckReservedLock(OsFile *id){
  int rc;
  winFile *pFile = (winFile*)id;
  assert( pFile!=0 );
...
*/
int winUnlock(OsFile *id, int locktype){
  int type;
  int rc = SQLITE_OK;
  winFile *pFile = (winFile*)id;
  assert( pFile!=0 );
...
*/
void winSetFullSync(OsFile *id, int v){
  return;
}
...
** for win32.
*/
const IoMethod sqlite3WinIoMethod = {
  winClose,
  winOpenDirectory,
  winRead,
  winWrite,
