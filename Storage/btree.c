...
** Forward declaration
*/
 int checkReadLocks(BtShared*,Pgno,BtCursor*);

/*
** Read or write a two- and four-byte big-endian integer values.
*/
 u32 get2byte(unsigned char *p){
  return (p[0]<<8) | p[1];
}
 u32 get4byte(unsigned char *p){
  return (p[0]<<24) | (p[1]<<16) | (p[2]<<8) | p[3];
}
 void put2byte(unsigned char *p, u32 v){
  p[0] = v>>8;
  p[1] = v;
}
 void put4byte(unsigned char *p, u32 v){
  p[0] = v>>24;
  p[1] = v>>16;
  p[2] = v>>8;
  p[3] = v;
...
*/
 int restoreOrClearCursorPositionX(BtCursor *pCur, int doSeek){
  int rc = SQLITE_OK;
...
*/
 u8 *findCell(MemPage *pPage, int iCell){
  u8 *data = pPage->aData;
  assert( iCell>=0 );
...
*/
 ...
 void _pageIntegrity(MemPage *pPage){
  int usableSize;
  u8 *data;
...
*/
 int getPage(BtShared *pBt, Pgno pgno, MemPage **ppPage){
  int rc;
  unsigned char *aData;
  MemPage *pPage;
...
** happens.
*/
 void pageDestructor(void *pData, int pageSize){
  MemPage *pPage;
  assert( (pageSize & 7)==0 );
...
*/
 void pageReinit(void *pData, int pageSize){
  MemPage *pPage;
  assert( (pageSize & 7)==0 );
  pPage = (MemPage*)&((char*)pData)[pageSize];
  if( pPage->isInit ){
...
int sqlite3BtreeBeginTrans(Btree *p, int wrflag){
  ...
  */
  ...
  }while( rc==SQLITE_BUSY && pBt->inTransaction==TRANS_NONE && SQLITE_OK );
...
/* Forward declaration required by autoVacuumCommit(). */
 int allocatePage(BtShared *, MemPage **, Pgno *, Pgno, u8);
...
*/
 ...
 int autoVacuumCommit(BtShared *pBt, Pgno *nTrunc){
  Pager *pPager = pBt->pPager;
...
** function above.
*/
 void releaseTempCursor(BtCursor *pCur){
  if( pCur->pPage ){
...
*/
 const unsigned char *fetchPayload(
  BtCursor *pCur,      /* Cursor pointing to entry to read from */
...
*/
 int checkReadLocks(BtShared *pBt, Pgno pgnoRoot, BtCursor *pExclude){
/*  BtCursor *p;
  for(p=pBt->pCursor; p; p=p->pNext){
...
*/
 int btreePageDump(BtShared *pBt, int pgno, int recursive, MemPage *pParent){
  int rc;
  MemPage *pPage;
  int i, j, c;
  int nFree;
