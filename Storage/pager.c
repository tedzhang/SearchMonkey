...
*/
void put32bits(char *ac, u32 val){
  ac[0] = (val>>24) & 0xff;
  ac[1] = (val>>16) & 0xff;
  ac[2] = (val>>8) & 0xff;
  ac[3] = val & 0xff;
...
** page header 'p'.
*/
u32 retrieve32bits(PgHdr *p, int offset){
  unsigned char *ac;
...
void page_remove_from_stmt_list(PgHdr *pPg){
  if( !pPg->inStmt ) return;
  if( pPg->pPrevStmt ){
...
*/
int pager_unwritelock(Pager *pPager){
  PgHdr *pPg;
  int rc;
  assert( !MEMDB );
  if( pPager->state<PAGER_RESERVED ){
...
** back from the disk.
*/
int pager_reload_cache(Pager *pPager){
  PgHdr *pPg;
  int rc = SQLITE_OK;
  for(pPg=pPager->pAll; pPg; pPg=pPg->pNextAll){
...
** closed.
*/
int sqlite3pager_opentemp(char *zFile, OsFile **pFd){
  int cnt = 8;
  int rc;
#ifdef SQLITE_TEST
...
*/
void memoryTruncate(Pager *pPager){
  PgHdr *pPg;
  PgHdr **ppPg;
...
int pager_wait_on_lock(Pager *pPager, int locktype){
  ...
  if( pPager->state>=locktype ){
    ...
    }while( rc==SQLITE_BUSY && SQLITE_OK/*sqlite3InvokeBusyHandler(pPager->pBusyHandler)*/ );
    if( rc==SQLITE_OK ){
